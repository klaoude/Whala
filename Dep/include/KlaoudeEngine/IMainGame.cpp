...
namespace KlaoudeEngine
{
	...
	{
		if (!init(windowName, screenWidth, screenHeight)) return;
	...
	bool IMainGame::init(std::string windowName, int screenWidth, int screenHeight)
	{
		...
		if (!initSystems(windowName, screenWidth, screenHeight)) 
			return false;
	...
	bool IMainGame::initSystems(std::string windowName, int screenWidth, int screenHeight)
	{
		m_window.create(windowName, screenWidth, screenHeight, 0);
		return true;
	}
}